...
$response->attributes()
    ...
    ->assertSame('price', 29.99)
    ->assertNotNull('created_at')
    ->assertNull('deleted_at')
    ->assertHasKey('sku')
    ->assertMissingKey('internal_notes');
```

### Verifying Relationships

```php
$response = $this->getJson('/api/orders/1?include=customer,items');

$response->assertOk();
$response->data()
    ->relationships()
    ->assertHasKey('customer')
    ->assertHasKey('items');
...
### Verifying Links and Meta on Items

```php
$response = $this->getJson('/api/products');

$response->assertOk();
$response->data()->item(0)->links()->assertHasKey('self');
$response->data()->item(0)->meta()->assertSame('stock_level', 'in_stock');
```
